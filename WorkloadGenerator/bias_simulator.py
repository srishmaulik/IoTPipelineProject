...
indoor_temperature = random.uniform(18, 24) + 5
#outdoor_temperature = random.uniform(15, 30)  
indoor_humidity = random.uniform(30, 60) + 10
#outdoor_humidity = random.uniform(20, 70)  
atmospheric_pressure = random.uniform(1010, 1020) + 5 
indoor_light = random.uniform(100, 500) + 100
...
def simulate_data():
    ...
    indoor_temperature = update_value(indoor_temperature, 18, 24) + 5
    #outdoor_temperature = update_value(outdoor_temperature, 15, 30)
    indoor_humidity = update_value(indoor_humidity, 30, 60) + 10
    #outdoor_humidity = update_value(outdoor_humidity, 20, 70)
    atmospheric_pressure = update_value(atmospheric_pressure, 1010, 1020) + 5
    indoor_light = update_value(indoor_light, 100, 500) + 100
    #outdoor_light = update_value(outdoor_light, 5000, 10000)
    battery_level = update_battery(battery_level)

def main():
    print("Starting Bias Simulator:", system_id)
    while(True):
        simulate_data()
        sensorTag = {
            "system_id":  system_id,
