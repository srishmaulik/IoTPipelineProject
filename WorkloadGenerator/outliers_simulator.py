...
def simulate_data():
    ...
    if random.uniform(0, 10) < 1.0:
        indoor_temperature = update_value(indoor_temperature, 18, 24) + 6
        #outdoor_temperature = update_value(outdoor_temperature, 15, 30)
        indoor_humidity = update_value(indoor_humidity, 30, 60) + 30
        #outdoor_humidity = update_value(outdoor_humidity, 20, 70)
        atmospheric_pressure = update_value(atmospheric_pressure, 1010, 1020) + 10
        indoor_light = update_value(indoor_light, 100, 500) + 400
...
def main():
    print("Starting Outlier Simulator:", system_id)
    while(True):
        simulate_data()
        sensorTag = {
            "system_id":  system_id,
